...
(function($) {
    ...
    function createDualListBox(options) {
        ...
        var style = 'height: 200px; width: 100%; padding: 5px;';
        $(options.parentElement).append(
                (options.horizontal == false ? '   <div class="col-md-5">' : '   <div class="col-md-6">') +
                '       <h4><span class="unselected-title"></span> <small>- '+options.showingLabel+' <span class="unselected-count"></span> 条记录</small></h4>' +
                '       <input class="filter form-control filter-unselected" type="text" placeholder="'+options.filterLabel+'" style="margin-bottom: 5px;">' +
                (options.horizontal == false ? '' : createHorizontalButtons(1, options.moveAllBtn)) +
                '       <select class="unselected" style="'+style+'" multiple></select>' +
                '   </div>' +
                (options.horizontal == false ? createVerticalButtons(options.moveAllBtn) : '') +
                (options.horizontal == false ? '   <div class="col-md-5">' : '   <div class="col-md-6">') +
                '       <h4><span class="selected-title"></span> <small>- '+options.showingLabel+' <span class="selected-count"></span> 条记录</small></h4>' +
                '       <input class="filter form-control filter-selected" type="text" placeholder="'+options.filterLabel+'" style="margin-bottom: 5px;">' +
                (options.horizontal == false ? '' : createHorizontalButtons(2, options.moveAllBtn)) +
                '       <select class="selected" style="'+style+'" multiple></select>' +
                '   </div>');

        $(options.parentElement + ' .selected').prop('name', $(options.element).prop('name'));
        $(options.parentElement + ' .unselected-title').text(options.availableLabel + options.title);
        $(options.parentElement + ' .selected-title').text(options.selectedLabel + options.title);
    }
